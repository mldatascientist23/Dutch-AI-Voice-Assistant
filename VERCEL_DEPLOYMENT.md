...
## 🚀 Deploying Dutch AI Voice Assistant on Vercel

This guide explains how to deploy the frontend of the Dutch AI Voice Assistant on Vercel.
...
Vercel is excellent for hosting the frontend, but the backend needs to be deployed on a platform that supports Python applications (like Render, Railway, or your own server).
...
4. **Configure project**:
   ...
   - Root Directory: ./
   - Build Command: `node build.js` (will be auto-detected from vercel.json)
...
1. Update `BACKEND_API_URL` in Vercel
2. Redeploy
...
2. Ensure Node.js is available (Vercel default)
...
- Set up different backend URLs for production vs preview
- Monitor usage on free tiers (both Vercel and backend platform)
...
- [ ] Visited Vercel URL - no 404 error
...
**🎉 Congratulations! Your Dutch AI Voice Assistant is now live on Vercel!**
